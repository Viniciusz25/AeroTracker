...
class DeviceCircleDisplay(QWidget):
    ...
    def __init__(self, parent=None) -> None:
        ...
        self.setMinimumSize(295, 295)
        self.setMaximumSize(325, 325)
    ...
    def paintEvent(self, event) -> None:
        ...
        glow_pen = QPen(QColor(71, 243, 160, 45), 14)
        painter.setPen(glow_pen)
        painter.setBrush(Qt.BrushStyle.NoBrush)
        painter.drawEllipse(QPointF(center_x, center_y), radius + 4, radius + 4)

        # 2. Tela Circular Preto Absoluto (AMOLED)
        painter.setPen(QPen(QColor(Theme.Colors.BORDER), 1.5))
        painter.setBrush(QBrush(QColor("#040507")))
        ...
        p2 = QPointF(center_x - 5, top_y + 4)
        p3 = QPointF(center_x + 5, top_y + 4)
        ...
        painter.drawText(QRectF(center_x - 60, top_y + 8, 120, 18), Qt.AlignmentFlag.AlignCenter, self.flight_number)

        # 4. Continentes Táticos Escuros (Orthographic Projection)
        map_pen = QPen(QColor("#131B18"), 1)
        map_brush = QBrush(QColor("#0B120F"))
        painter.setPen(map_pen)
        painter.setBrush(map_brush)

        # Ásia / Coreia (Esquerda)
        asia = QPainterPath()
        asia.addEllipse(center_x - radius + 15, center_y - 45, 60, 75)
        painter.drawPath(asia)

        # América do Norte (Direita)
        na = QPainterPath()
        na.addEllipse(center_x + radius - 75, center_y - 65, 65, 85)
        painter.drawPath(na)

        # 5. Arco de Voo Tracejado em Verde Neon (ICN -> SFO)
        arc_path = QPainterPath()
        arc_x1 = center_x - radius + 40
        arc_y1 = center_y + 5
        arc_x2 = center_x + radius - 40
        arc_y2 = center_y - 10
        arc_path.moveTo(arc_x1, arc_y1)
        arc_path.quadTo(center_x, center_y - radius + 50, arc_x2, arc_y2)
        ...
        painter.setPen(QPen(QColor(Theme.Colors.TEXT_PRIMARY)))
        painter.drawText(QRectF(arc_x1 - 35, arc_y1 - 12, 30, 16), Qt.AlignmentFlag.AlignCenter, self.origin_code)
        painter.drawText(QRectF(arc_x2 + 5, arc_y2 - 12, 30, 16), Qt.AlignmentFlag.AlignCenter, self.dest_code)
        ...
        painter.setPen(Qt.PenStyle.NoPen)
        painter.setBrush(QBrush(QColor(Theme.Colors.PRIMARY)))
        painter.drawEllipse(QPointF(center_x - 8, center_y + radius - 18), 3, 3)
        painter.setBrush(QBrush(QColor(Theme.Colors.TEXT_MUTED)))
        painter.drawEllipse(QPointF(center_x, center_y + radius - 18), 2.5, 2.5)
        painter.drawEllipse(QPointF(center_x + 8, center_y + radius - 18), 2.5, 2.5)
...
class DevicePreviewPanel(QFrame):
    ...
    def __init__(self, parent=None) -> None:
        ...
        lbl_head.setStyleSheet(f"color: {Theme.Colors.TEXT_MUTED}; border: none; font-size: 8px; letter-spacing: 1px;")
        ...
        lbl_swipe.setStyleSheet(f"color: {Theme.Colors.TEXT_MUTED}; border: none; font-size: 8px;")
        ...
        s_layout.addLayout(self._create_status_row("DEVICE", "Pushed Cartographic Glass to ESP32-S3"))
    ...
    def _create_status_row(self, tag: str, text: str) -> QHBoxLayout:
        ...
        lbl_tag.setStyleSheet(f"color: {Theme.Colors.TEXT_MUTED}; border: none; font-size: 8px; font-weight: bold;")
        lbl_tag.setFixedWidth(50)

        lbl_val = QLabel(f"🟢 {text}")
        lbl_val.setFont(Theme.Fonts.caption())
        lbl_val.setStyleSheet(f"color: {Theme.Colors.PRIMARY}; border: none; font-size: 9px;")
